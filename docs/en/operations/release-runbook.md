...
npm run version:check -- --tag v0.1.7
```
...
```powershell
gh attestation verify .\AgentMux_0.1.7_x64-setup.exe --repo raeseoklee/agentmux --signer-workflow raeseoklee/agentmux/.github/workflows/release.yml
```

Verify hash:

```powershell
Get-FileHash -Algorithm SHA256 .\AgentMux_0.1.7_x64-setup.exe
Get-Content .\AgentMux_0.1.7_x64-setup.exe.sha256
```
