...
AgentMux는 Windows 전용 AI 에이전트 워크플로 Terminal 멀티플렉서입니다.
여러 Terminal, 에이전트, Browser surface, Workspace 상태를 한 앱에서
...
- Workspace, Tab, 분할 Pane, Session 메타데이터를 SQLite에 저장.

## 에이전트 워크플로

- 액션, 명령 팔레트, CLI를 통한 에이전트 실행.
- running, waiting for input, completed, failed 상태 감지.
- 입력 대기나 실패처럼 개입이 필요한 에이전트를 Workspace, Pane,
  알림, OS 알림으로 표시.
- 앱 재시작 후 가능한 경우 에이전트 실행 명령과 작업 디렉터리를 복원.

## Workspace 레이아웃

- Workspace, 상단 Tab, split Pane 지원.
- Tab은 각자 독립적인 Pane 레이아웃을 소유하며, 새 Tab 추가가 기존 split
  트리를 변경하지 않음.
- Pane split, resize, focus, close, surface mount/unmount.
- Workspace, Tab, Pane의 이동과 순서 변경.

## 브라우저와 자동화

- Terminal 옆에 Browser surface를 split Pane으로 열기.
- 현재 선택 영역, 알림, 팀 메시지, 팀 task, attention reason에서 URL을
  찾아 AgentMux 브라우저 탭으로 여는 `browser.openContextLink` 액션.
- Terminal 출력의 OSC 8 링크와 일반 `http://` / `https://` URL을 Windows에서
  Ctrl-click으로 열기.
...
- Server mode는 데스크톱과 같은 React Workspace UI를 사용하며, 선택한
  Pane의 경로와 backend를 기준으로 Git branch/hash와 Source Control을 표시.
...
한국어 UI와 문서에서도 `Workspace`, `Tab`, `Pane`, `Terminal`, `Git`, `MCP`,
`Browser`는 AgentMux의 고유 개념을 일관되게 가리키기 위해 영문 표기를
유지합니다.
