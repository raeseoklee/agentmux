...
export function SourceControlPanel({ client, workspace, activePaneId, activeSessionId, activeCwd, onClose, onRepositoryChanged, t }: Props) {
  ...
  const loadThreads = useCallback(async (path?: string | null, repositoryId?: string | null) => {
    ...
    try {
      ...
      if (sequence === threadSequence.current) setThreads(nextThreads);
    } catch (cause) {
      if (sequence === threadSequence.current) setError(cause instanceof Error ? cause.message : String(cause));
    }
  }, [client, workspace.workspaceId]);
  ...
  const refresh = useCallback(async (showLoading = false) => {
    ...
    try {
      const page = await client.getGitStatusPage(workspace.workspaceId, {
        paneId: activePaneId, limit: PAGE_SIZE,
      });
  ...
  }, [activePaneId, client, loadWorktrees, workspace.workspaceId]);
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      if (operationTarget !== targetKeyRef.current || sequence !== refreshSequence.current) return;
      if (shouldReloadGitPage(summary.generation, page.generation)) { await refresh(); return; }
      setChanges((current) => [...current, ...page.changes]);
      setNextCursor(page.nextCursor ?? null);
      loadedGeneration.current = page.generation;
    } catch (cause) {
      if (operationTarget === targetKeyRef.current && sequence === refreshSequence.current) setError(cause instanceof Error ? cause.message : String(cause));
    } finally {
      if (operationTarget === targetKeyRef.current) setLoadingMore(false);
    }
  }, [activePaneId, client, loadingMore, nextCursor, refresh, summary, targetKey, workspace.workspaceId]);
  ...
  }, [client, refresh, targetKey, workspace.workspaceId]);
  ...
  }, [activePaneId, client, loadThreads, selection, summary, workspace.workspaceId]);
  ...
  const mutate = useCallback(async (operation: () => Promise<unknown>) => {
    if (busy) return;
    const operationTarget = targetKeyRef.current;
    setBusy(true);
    try {
      await operation();
    ...
    catch (cause) { if (operationTarget === targetKeyRef.current) setError(cause instanceof Error ? cause.message : String(cause)); }
  ...
  const commit = useCallback(async () => {
    ...
    try {
      ...
      if (operationTarget === targetKeyRef.current) setError(cause instanceof Error ? cause.message : String(cause));
    } finally {
      setBusy(false);
    }
  }, [activePaneId, busy, client, commitMessage, dialogs, onRepositoryChanged, refresh, stagedCount, t, workspace.workspaceId]);
