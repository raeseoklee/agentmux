...
const ZOOM_KEY = "agentmux.uiZoom";
...
export function defaultZoomForDisplay(): number {
  ...
  // DPR ≈ 1 → the OS draws 1:1, so scale by how dense the panel is.
  const width = window.screen?.width ?? 1920;
  if (width >= 3840) return 1.5; // UHD / 4K @ 100%
  if (width >= 2560) return 1.15; // QHD @ 100%
  if (width >= 1920) return 1.1; // FHD @ 100%
  return 1;
}
...
/** Apply a zoom factor to the webview and persist it. Returns the clamped value. */
